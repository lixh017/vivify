...
// returns N differentiated topic ideas generated by MiniMax for a
// given seed + platform. /ai/humanize rewrites AI-flavored scripts
// to read more naturally. /ai/postmortem walks a content item's FK
// chain to assemble context, then asks MiniMax for a structured
// retrospective.
...
// can pass nil.
func NewAIHandler(text *agents.MiniMax, db ...*gorm.DB) *AIHandler {
	l := slog.Default()
	var d *gorm.DB
	if len(db) > 0 {
		d = db[0]
	}
	return &AIHandler{text: text, db: d, logger: l}
}
...
// (?demo=true) or because no MiniMax API key is configured. The
// second branch keeps the dev / CI loop useful when MINIMAX_API_KEY
// is absent.
...
func (h *AIHandler) HumanizeScript(c *gin.Context) {
	...
	c.JSON(http.StatusOK, humanizeResponse{Humanized: res.Text})
}
...
func (h *AIHandler) Postmortem(c *gin.Context) {
	...
	structured := parsePostmortemStructured(res.Text)
	c.JSON(http.StatusOK, postmortemResponse{Report: res.Text, Structured: structured})
}
